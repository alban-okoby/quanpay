...
import com.quanpay.security.jwt.TokenAuthenticationFilter;
import com.quanpay.security.oauth2.CustomOAuth2UserService;
import com.quanpay.security.oauth2.CustomOidcUserService;
import com.quanpay.security.oauth2.HttpCookieOAuth2AuthorizationRequestRepository;
import com.quanpay.security.oauth2.OAuth2AccessTokenResponseConverterWithDefaults;
import com.quanpay.security.oauth2.OAuth2AuthenticationFailureHandler;
import com.quanpay.security.oauth2.OAuth2AuthenticationSuccessHandler;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.cors().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().csrf().disable().formLogin().disable().httpBasic().disable()
				...
				.antMatchers("/", "/error", "/api/v1/auth/**", "/oauth2/**").permitAll().anyRequest().authenticated().and().oauth2Login().authorizationEndpoint()
